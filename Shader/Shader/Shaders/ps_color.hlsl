...
{
    float2 dUV = -tsEyeVec.xy * 0.08; //~displm’tdepth
    float  prev_hits= 0; 
    float  hit_h= 0;       
    float  h = 0;
    int    steps = 0;
    ...
    { 
        h  -= 0.1f; 
        uv+= dUV; 
        float h_tex = displacementTexture.SampleLevel(SampleTypeWrap2, uv, 0).x;
        float is_first_hit= saturate( (h_tex - h - prev_hits)*4999999); 
        hit_h+= is_first_hit* h; 
        steps += it * is_first_hit;
...
float2 Displacement(float2 tsEyeVec, float2 uv)
{
    ...
    float2 NewCoords = uv;
    //float2 dUV = -tsEyeVec.xy * 0.1 * 0.08;
